...
export default class IndexHelper {
  ...
  static getT(type, row, col) {
    const N = IndexHelper.resolveN(type)
    return row * N + col;
  }

  static getRowCol(type, t) {
    const N = IndexHelper.resolveN(type)
    return [Math.floor(t / N), t % N];
  }

  static getPos(type, t) {
   ...
   */
  static *iterRowCol(type, rect, direction = 'down') {
    const N = IndexHelper.resolveN(type)
    const itemSize = IndexHelper.resolveItemSize(type)
  ...
  static *iter(type, rect, direction = 'down') {
    const N = IndexHelper.resolveN(type)

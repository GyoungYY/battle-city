...
export function lastPos(b) {
  return b.merge({ x: b.lastX, y: b.lastY })
}

export class BulletCollisionInfo extends DefaultMap {
  ...
  static getCollisionRect(collision) {
    const borderCollisionRect = {
      x: FIELD_SIZE,
      y: FIELD_SIZE,
      width: -FIELD_SIZE,
      height: -FIELD_SIZE,
  ...
  // 判断一颗子弹是否发生爆炸
  static shouldExplode(collisions) {
  ...
  getExplosionInfo() {
    const expBullets = new Map()
    const noExpBullets = new Map()
...
function calculateHitTime(b1, b2) {
  if (b1.direction === 'up') {
...
function moveFromLast(bullet, time) {
...
export function spreadBullet(bullet) {

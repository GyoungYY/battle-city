<template>
    <g className="text" :transform="`translate(${x},${y})`" :style="style">
        <g v-for="(char, i) in contentArray" :key="i" :transform="`translate(${8 * i},0)`">
            <template v-html="chars[char](fill)"></template>    
        </g>
    </g>
</template>
<script>
export default {
    props: ['content', 'x', 'y', 'fill', 'style'],
    data() {
        return {
            chars: {
                ' ': () => <g className="character-blank" />,
                0: ({ fill }) => (
                    return `<path className="character-0" fill=${fill} d="M3,0 h3  v1  h1  v1  h1  v3  h-1 v1  h-1 v-4 h-1 v-1 h-2 v-1 M6,7 h-3 v-1 h-1 v-1 h-1 v-3 h1  v-1 h1  v4  h1  v1  h2  v1"/>`
                ),
                1: ({ fill }) => (
    ...
    computed: {
        contentArray() {
            return Array.from(this.content.toLowerCase());
        }
    }
}
</script>
